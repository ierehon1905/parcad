...
inline double parcad_volume_fixed(const TopoDS_Shape& shape) {
  GProp_GProps props;
...
// face whose two differ by more than `rel` of the boundary's, or that carries
// no triangulation.
inline rust::Vec<double> parcad_uncovered_faces(const TopoDS_Shape& shape, double rel) {
  return parcad_surfacing_guard("checking the mesh covers every face", [&]() {
    ...
    for (int i = 1; i <= faces.Extent(); ++i) {
      ...
      // The boundary, walked edge by edge in the face's own direction.
      double boundary = 0.0;
      bool complete = true;
      ...
      for (TopExp_Explorer wires(forward, TopAbs_WIRE); wires.More(); wires.Next()) {
        for (TopExp_Explorer it(wires.Current(), TopAbs_EDGE); it.More(); it.Next()) {
          ...
          if (BRep_Tool::Degenerated(edge)) {
            // A pole: its polygon still bounds the parameter domain.
          }
          TopLoc_Location on;
          ...
          for (int k = 1; k < count; ++k) {
            ...
            boundary += p.X() * q.Y() - q.X() * p.Y();
          }
        }
      }
      boundary = std::abs(boundary) / 2.0;
      if (!complete || std::abs(meshed - boundary) > rel * boundary) {
...
inline std::unique_ptr<TopoDS_Shape> parcad_reversed(const TopoDS_Shape& shape) {
  return parcad_boxed(shape.Reversed());
}
...
// The solid a closed shell bounds, facing out and valid, or an error.
inline std::unique_ptr<TopoDS_Shape> parcad_solid_from_shell(const TopoDS_Shape& shape) {
  return parcad_surfacing_guard("closing the shell into a solid", [&]() {
    TopExp_Explorer shells(shape, TopAbs_SHELL);
    if (!shells.More()) {
      throw std::runtime_error("there is no shell to close");
    }
    const TopoDS_Shell shell = TopoDS::Shell(shells.Current());
    shells.Next();
    if (shells.More()) {
      throw std::runtime_error("the surfaces stitched into more than one shell");
    }
    BRepBuilderAPI_MakeSolid make(shell);
    if (!make.IsDone()) {
      throw std::runtime_error("the closed shell could not be made into a solid");
    }
    TopoDS_Solid solid = make.Solid();
    BRepLib::OrientClosedSolid(solid);
    BRepCheck_Analyzer check(solid);
    if (!check.IsValid()) {
      throw std::runtime_error("the closed shell's solid does not pass the kernel's validity check");
    }
    return parcad_boxed(solid);
  });
}
