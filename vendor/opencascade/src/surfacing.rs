...
fn samples(v: &[f64]) -> Vec<FaceSample> {
    v.chunks_exact(9)
        .map(|c| FaceSample {
            face: c[0] as usize,
...
impl Shape {
    ...
    /// two differ by more than `rel` of the boundary's plus the perimeter
    /// times the kernel's parametric confusion, or that carries no mesh.
    ...
    /// For every face, the point bending it most toward its normal and the
    /// point bending it most away: `per` points in each continuous stretch of
    /// its surface each way, the best of them climbed to the extreme.
    pub fn bend_extremes(&self, per: usize) -> Result<Vec<FaceSample>, String> {
        let v = ffi::parcad_bend_extremes(&self.inner, per as i32).map_err(|e| e.what().to_string())?;
        Ok(samples(&v))
    }
...
pub(crate) mod ffi {
    unsafe extern "C++" {
        ...
        fn parcad_bend_extremes(shape: &TopoDS_Shape, per: i32) -> Result<Vec<f64>>;
