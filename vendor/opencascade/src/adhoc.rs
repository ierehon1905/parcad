...
impl AdHocShape {
    ...
    /// Make a sphere of radius `r` centred on `p`.
    ///
    /// `BRepPrimAPI_MakeSphere` builds at the origin and takes no centre, so the
    /// translation is applied afterwards.
    pub fn make_sphere(p: DVec3, r: f64) -> Self {
        let mut sphere = ffi::BRepPrimAPI_MakeSphere_ctor(r);
        let inner = ffi::TopoDS_Shape_to_owned(sphere.pin_mut().Shape());

        Self::from_inner(inner).translate_to(p)
    }

    /// Move this shape by `by`, consuming it.
    fn translate_to(self, by: DVec3) -> Self {
        if by == DVec3::ZERO {
            return self;
        }
        Self(self.0.translated(by))
    }
