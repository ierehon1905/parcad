 ...
 * looking at.
 *
 * These used to live in the titlebar, beside Save and Export. That grouping
 * was wrong in a way worth naming, because the fix is not cosmetic: **none of
 * them changes the part.** Save writes a file, Export writes a file, and the
 * picker chooses which file; those belong to the document. A section plane
 * belongs to the eye. Putting them all in one row said they were the same
 * kind of thing, and the row was crowded enough that the second author — the
 * agent editing the same file — got pushed to the far edge of the window.
 *
 * So they moved onto the viewport, which is also where every other CAD program
 * puts them — and on the way, two of the three turned out not to exist.
 *
 * **The mesh-detail slider is gone, not moved.** It was there for a long time,
 * dimmed under B-rep, and reading the host settled what it actually did: the
 * exact kernel has "no grid to coarsen and no use for a requested depth", so
 * the slider moved a number nothing read every time it was dragged. Dimming it
 * under one kernel had disguised that as a mode rather than a defect.
 *
 * **The kernel toggle is gone too.** It offered "exact" and "preview", and
 * both meshed the same exact solid; the second only dropped the edges and the
 * face pick from what came back. With one kernel there is nothing to choose.
 *
 ...
 * rule the deletions above came from: show the controls that apply to the
 * state the app is actually in, and if a control never applies, it is not a
 * control.
 *
