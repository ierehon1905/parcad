import{L as te,F as j,S as A,a as ne,B as oe,b as W,C as ae,c as se}from"./index-C2NArZHl.js";const re=`// parcad — everything is millimetres, Z is up.
...
`,ie=`// A cast machine foot: a drafted pedestal on a drafted base, bolted down
...
return machined
  ...
  .chamfer(0.8)
  .tag("seat_deburr");
`,he=`// A threaded rod-end clevis: a shank with spanner flats, and a two-armed fork
// for a 10 mm pin.
//
...
return machined
  ...
  .chamfer(0.5)
  .tag("pin_lead_in");
`,le=`// A bolted cover plate with a turned spigot and countersunk screws.
//
...
return machined
  ...
  .chamfer(0.4)
  .tag("top_face_deburr");
`,ce=`// diamond v19 — recreated from a Fusion 360 export.
//
...
`,de=`// An instrument fascia: a display module drops into a milled seat in the front
...
return machined
  ...
  .chamfer(0.4)
  .tag("seat_lead_in");
`,ue=`// Selected edge treatments. The queries are geometric, not \`edge[7]\`:
...
return rounded
  ...
  .chamfer(1)
  .tag("edge_treatments");
`,pe=`// A printable enclosure — shows shell() and offset().
...
`,me=`// A 200 mm length of 20x20 T-slot aluminium extrusion.
//
...
return corners
  ...
  .tag("corner_radii");
`,ge=`// Control box: a moulded enclosure for a small board, in the field-instrument
...
`,be=`// Desk stand: a phone rest milled from one slab, in the field-instrument
...
return stand;
`,fe=`// Field planter: a wide pot standing on a vented plinth, and a tray it sits in.
//
...
return { pot, tray };
`,we=`// A slip-on pipe flange, dimensioned after ASME B16.5 class 150, NPS 2.
//
...
`,ye=`// Retainer v1 — recreated from a Fusion 360 export.
//
...
`,ve=`// Untitled2 v1 — recreated from the Fusion 360 export.
//
...
`,ke=`// UnTriangle v3 — recreated from the Fusion 360 export.
//
...
`,xe=`// An extruded-profile heat sink, 60 x 60, with a plain fin array.
//
...
`,Te=`// An M3 hex standoff, 5.5 mm across the flats, 20 mm long, bored 2.5 mm for
// a tapped thread.
//
...
return drilled
  ...
  .chamfer(0.4)
  .tag("thread_lead_in");
`,_e=`// A bent hydraulic line with a flare fitting boss at each end, and the O-ring
...
return machined
  ...
  .chamfer(0.5)
  .tag("seat_chamfer");
`,De=`// A knurled control knob for a 6 mm shaft with a flat — the D-bore that stops
...
return bored
  ...
  .chamfer(0.5)
  .tag("bore_lead_in");
`,Ae=`// Lidded box: an open base and a lid with a locating lip, printed as two parts.
//
...
return { base, lid };
`,Re=`// A hydraulic manifold block: a solid with cross-drilled galleries that meet
...
`,Se=`// A NEMA 17 stepper motor mount: an L-bracket whose face plate carries the
...
`,Le=`// A pillow block for a 20 mm bore ball bearing (a UCP-204 style housing,
...
`,He=`// A socket-weld pipe tee for 1" schedule 40 pipe.
//
...
`,Me=`// A vertical dinner-plate stand for a cupboard shelf, drawn so the pegs stop
// breaking off.
//
...
return slotted
  ...
  .chamfer(0.8)
  .tag("slot_rims");
`,ze=`// A pleated pendant shade: two six-pointed stars, each divided evenly and
...
`,Ee=`// Screw-top jar: a jar with a threaded neck and its cap, printed as two parts.
//
...
return { jar, cap };
`,Pe=`// A rigid set-screw shaft coupler: 8 mm motor shaft to 10 mm leadscrew.
//
...
`,Oe=`// A spur gear pair, module 2, 20 and 30 teeth, meshed at their centre
...
return { pinion, wheel };
`,Ce=`// A 20-tooth GT2 timing pulley for 6 mm belt, on a 5 mm motor shaft.
//
...
return machined
  ...
  .chamfer(0.4)
  .tag("bore_lead_in");
`,Ie=`// Twisted planter and its drip saucer, printed as two parts.
//
...
`,Be=`// A toolroom V-block: a 90° vee that holds round stock on its axis, whichever
// diameter it is.
//
...
return machined
  ...
  .chamfer(1)
  .tag("vee_lip_break");
`,Fe=`// A wash bottle: a turned body whose outline is one section — straight
...
`;var T={script:"kernel/7910a78de525/parcad-wasm.js",wasm:"kernel/7910a78de525/parcad_wasm.wasm",bytes:21080335};const M=new Set;let _={phase:"downloading",received:0,total:T.bytes};function k(n){_=n;for(const e of M)e(_)}function Ne(n){return M.add(n),n(_),()=>M.delete(n)}let R;function G(){return R??=(async()=>{const n=new URL(T.wasm,document.baseURI).href,e=await fetch(n);if(!e.ok||!e.body)throw new Error(`the geometry kernel did not download (${e.status} from ${n}); reload the page to try again`);const o=T.bytes,s=e.body.getReader(),r=[];let a=0;for(;;){const{done:l,value:d}=await s.read();if(l)break;r.push(d),a+=d.length,k({phase:"downloading",received:a,total:o})}k({phase:"compiling",received:a,total:o});const t=new Uint8Array(a);let i=0;for(const l of r)t.set(l,i),i+=l.length;const h=await WebAssembly.compile(t);return k({phase:"ready",received:a,total:o}),h})().catch(n=>{R=void 0;const e=n instanceof Error?n.message:String(n);throw k({..._,phase:"failed",error:e}),new Error(`${e}
This browser could not start the WebAssembly kernel. It needs WebAssembly exception handling: Chrome 95, Firefox 100 or Safari 15.2, or newer.`)}),R}function qe(){G().catch(()=>{})}let D,je=0,X=Promise.resolve();async function We(){const n=await G(),e=new Worker(new URL("/parcad/assets/kernel-worker-DMy80BGU.js",import.meta.url),{type:"module"}),o={worker:e,stage:"starting up",ready:Promise.resolve()};o.ready=new Promise((r,a)=>{const t=i=>a(new Error(`the geometry kernel did not start in this tab (${i}); reload the page`));e.onmessage=i=>{i.data.kind==="ready"&&r(),i.data.kind==="died"&&t(i.data.detail)},e.onerror=i=>t(i.message)});const s=new URL(T.script,document.baseURI).href;return e.postMessage({kind:"start",module:n,script:s}),await o.ready,o}function S(){D?.worker.terminate(),D=void 0}function $(n,e){const o=X.then(()=>Xe(n,e));return X=o.catch(()=>{}),o}async function Xe(n,e){try{D??=await We()}catch(a){return S(),{kind:"unavailable",message:a instanceof Error?a.message:String(a)}}const o=D,s=je++;o.stage="reading the request";const r=performance.now();return new Promise(a=>{const t=window.setTimeout(()=>{S(),a({kind:"timed_out",stage:o.stage,seconds:Math.round(e/1e3)})},e),i=h=>{window.clearTimeout(t),h.kind!=="replied"&&S(),a(h)};o.worker.onmessage=h=>{const l=h.data;l.kind==="stage"?o.stage=l.stage:l.kind==="died"?i({kind:"crashed",stage:o.stage,detail:l.detail}):l.kind==="reply"&&l.id===s&&i({kind:"replied",packet:l.packet,ms:Math.round(performance.now()-r)})},o.worker.onerror=h=>i({kind:"crashed",stage:o.stage,detail:h.message||"the worker failed"}),o.worker.postMessage({kind:"call",id:s,packet:n},[n.buffer])})}const ht=Object.freeze(Object.defineProperty({__proto__:null,preload:qe,run:$,watchLoad:Ne},Symbol.toStringTag,{value:"Module"}));var Y={script:"kernel/7910a78de525/parcad-wasm-host.js",wasm:"kernel/7910a78de525/parcad_wasm_host.wasm"};const Ye=Object.assign({"../../../examples/bracket.js":re,"../../../examples/cast-foot.js":ie,"../../../examples/clevis.js":he,"../../../examples/cover-plate.js":le,"../../../examples/diamond-v19.js":ce,"../../../examples/display-bezel.js":de,"../../../examples/edge-fillets.js":ue,"../../../examples/enclosure.js":pe,"../../../examples/extrusion-2020.js":me,"../../../examples/field-instrument/control-box.js":ge,"../../../examples/field-instrument/desk-stand.js":be,"../../../examples/field-instrument/planter.js":fe,"../../../examples/flange.js":we,"../../../examples/fusion360/retainer-v1.js":ye,"../../../examples/fusion360/untitled2-v1.js":ve,"../../../examples/fusion360/untriangle-v3.js":ke,"../../../examples/heat-sink.js":xe,"../../../examples/hex-standoff.js":Te,"../../../examples/hydraulic-line.js":_e,"../../../examples/knurled-knob.js":De,"../../../examples/lidded-box.js":Ae,"../../../examples/manifold-block.js":Re,"../../../examples/motor-mount.js":Se,"../../../examples/pillow-block.js":Le,"../../../examples/pipe-tee.js":He,"../../../examples/plate-stand.js":Me,"../../../examples/pleated-shade.js":ze,"../../../examples/screw-top-jar.js":Ee,"../../../examples/shaft-coupler.js":Pe,"../../../examples/spur-gears.js":Oe,"../../../examples/timing-pulley.js":Ce,"../../../examples/twisted-planter.js":Ie,"../../../examples/v-block.js":Be,"../../../examples/wash-bottle.js":Fe}),Ue="twisted-planter";let z,x,Ze=0;const E=new Map,P=new Set,O=new Set,C=new Set;let I={kind:"off"},B=null;const L=n=>new URL(n,document.baseURI).href;function Ge(){const n=new Worker(new URL("/parcad/assets/host-worker-CFFWifol.js",import.meta.url),{type:"module"}),e=new Promise((s,r)=>{n.onmessage=a=>{const t=a.data;switch(t.kind){case"ready":s(n),B&&n.postMessage({kind:"relay",...B});break;case"failed":r(new Error(`this tab could not start its host: ${t.error}`));break;case"reply":E.get(t.id)?.(t.reply),E.delete(t.id);break;case"kernel":$(t.packet,t.timeoutMs).then(i=>n.postMessage({kind:"kernel-end",ticket:t.ticket,end:i},i.kind==="replied"?[i.packet.buffer]:[]));break;case"events":for(const i of t.events)if(i.kind==="session")for(const h of P)h(i.session);else if(i.kind==="mcp")for(const h of O)h();break;case"download":Ke(new Blob([t.bytes]),t.name);break;case"relay":I=t.state;for(const i of C)i(I);break}},n.onerror=a=>r(new Error(`this tab's host did not load (${a.message})`))}),o=Object.fromEntries(Object.entries(Ye).map(([s,r])=>[s.replace(/^.*\/examples\//,"").replace(/\.js$/,""),r]));return n.postMessage({kind:"start",script:L(Y.script),wasm:L(Y.wasm),viewer:L("viewer.html"),seeds:o,preferred:Ue}),z=n,e}function N(){return x??=Ge().catch(n=>{throw x=void 0,n}),x}function K(){N().catch(()=>{})}async function J(n,e,o){const s=await N(),r=Ze++;return new Promise(a=>{E.set(r,t=>{t.kind==="refused"&&t.status===500&&t.error.startsWith("the host failed inside the tab")&&z===s&&(s.terminate(),z=void 0,x=void 0),a(t)}),s.postMessage({kind:"call",id:r,input:{route:{method:e,path:n,body:o}}})})}async function $e(n,e={}){const o=(e.method??"GET").toUpperCase(),s=typeof e.body=="string"?JSON.parse(e.body):null,r=n.split("/").map(decodeURIComponent).join("/"),a=await J(r,o,s);switch(a.kind){case"json":return new Response(JSON.stringify(a.json),{headers:{"content-type":"application/json"}});case"bytes":return new Response(a.bytes,{headers:{"content-type":a.contentType}});case"refused":return new Response(JSON.stringify({error:a.error}),{status:a.status,headers:{"content-type":"application/json"}});case"meshed":return new Response(JSON.stringify(Q(a.bytes)),{headers:{"content-type":"application/json"}})}}async function V(n){const e=await J("evaluate","POST",{graph:n});if(e.kind==="refused")throw new Error(e.error);if(e.kind!=="meshed")throw new Error("the host answered an evaluation without its mesh");return Q(e.bytes)}function Q(n){const e=n.buffer.slice(n.byteOffset,n.byteOffset+n.byteLength),[o,s,r,a]=new Uint32Array(e,0,4),t=JSON.parse(new TextDecoder().decode(new Uint8Array(e,16,o)));let i=16+Math.ceil(o/4)*4;const h=l=>e.slice(i,i+=4*l);return t.positions=new Float32Array(h(s)),t.normals=new Float32Array(h(r)),t.indices=new Uint32Array(h(a)),t}function Ke(n,e){const o=URL.createObjectURL(n),s=document.createElement("a");s.href=o,s.download=e,s.click(),setTimeout(()=>URL.revokeObjectURL(o),1e4)}function Je(n){return P.add(n),K(),()=>P.delete(n)}function Ve(n){return O.add(n),()=>O.delete(n)}function Qe(n){return C.add(n),n(I),()=>C.delete(n)}async function et(n,e){B=n?{relay:n,key:e}:null,(await N()).postMessage({kind:"relay",relay:n,key:e})}const lt=Object.freeze(Object.defineProperty({__proto__:null,evaluate:V,onMcp:Ve,onSession:Je,preload:K,request:$e,useRelay:et,watchRelay:Qe},Symbol.toStringTag,{value:"Module"})),H=new WeakMap;class tt extends te{constructor(e){super(e),this.decoderPath="",this.decoderConfig={},this.decoderBinary=null,this.decoderPending=null,this.workerLimit=4,this.workerPool=[],this.workerNextTaskID=1,this.workerSourceURL="",this.defaultAttributeIDs={position:"POSITION",normal:"NORMAL",color:"COLOR",uv:"TEX_COORD"},this.defaultAttributeTypes={position:"Float32Array",normal:"Float32Array",color:"Float32Array",uv:"Float32Array"}}setDecoderPath(e){return this.decoderPath=e,this}setDecoderConfig(e){return this.decoderConfig=e,this}setWorkerLimit(e){return this.workerLimit=e,this}load(e,o,s,r){const a=new j(this.manager);a.setPath(this.path),a.setResponseType("arraybuffer"),a.setRequestHeader(this.requestHeader),a.setWithCredentials(this.withCredentials),a.load(e,t=>{this.parse(t,o,r)},s,r)}parse(e,o,s=()=>{}){this.decodeDracoFile(e,o,null,null,A,s).catch(s)}decodeDracoFile(e,o,s,r,a=ne,t=()=>{}){const i={attributeIDs:s||this.defaultAttributeIDs,attributeTypes:r||this.defaultAttributeTypes,useUniqueIDs:!!s,vertexColorSpace:a};return this.decodeGeometry(e,i).then(o).catch(t)}decodeGeometry(e,o){const s=JSON.stringify(o);if(H.has(e)){const h=H.get(e);if(h.key===s)return h.promise;if(e.byteLength===0)throw new Error("THREE.DRACOLoader: Unable to re-decode a buffer with different settings. Buffer has already been transferred.")}let r;const a=this.workerNextTaskID++,t=e.byteLength,i=this._getWorker(a,t).then(h=>(r=h,new Promise((l,d)=>{r._callbacks[a]={resolve:l,reject:d},r.postMessage({type:"decode",id:a,taskConfig:o,buffer:e},[e])}))).then(h=>this._createGeometry(h.geometry));return i.catch(()=>!0).then(()=>{r&&a&&this._releaseTask(r,a)}),H.set(e,{key:s,promise:i}),i}_createGeometry(e){const o=new oe;e.index&&o.setIndex(new W(e.index.array,1));for(let s=0;s<e.attributes.length;s++){const r=e.attributes[s],a=r.name,t=r.array,i=r.itemSize,h=new W(t,i);a==="color"&&(this._assignVertexColorSpace(h,r.vertexColorSpace),h.normalized=!(t instanceof Float32Array)),o.setAttribute(a,h)}return o}_assignVertexColorSpace(e,o){if(o!==A)return;const s=new ae;for(let r=0,a=e.count;r<a;r++)s.fromBufferAttribute(e,r),se.colorSpaceToWorking(s,A),e.setXYZ(r,s.r,s.g,s.b)}_loadLibrary(e,o){const s=new j(this.manager);return s.setPath(this.decoderPath),s.setResponseType(o),s.setWithCredentials(this.withCredentials),new Promise((r,a)=>{s.load(e,r,void 0,a)})}preload(){return this._initDecoder(),this}_initDecoder(){if(this.decoderPending)return this.decoderPending;const e=typeof WebAssembly!="object"||this.decoderConfig.type==="js",o=[];return e?o.push(this._loadLibrary("draco_decoder.js","text")):(o.push(this._loadLibrary("draco_wasm_wrapper.js","text")),o.push(this._loadLibrary("draco_decoder.wasm","arraybuffer"))),this.decoderPending=Promise.all(o).then(s=>{const r=s[0];e||(this.decoderConfig.wasmBinary=s[1]);const a=nt.toString(),t=["/* draco decoder */",r,"","/* worker */",a.substring(a.indexOf("{")+1,a.lastIndexOf("}"))].join(`
`);this.workerSourceURL=URL.createObjectURL(new Blob([t]))}),this.decoderPending}_getWorker(e,o){return this._initDecoder().then(()=>{if(this.workerPool.length<this.workerLimit){const r=new Worker(this.workerSourceURL);r._callbacks={},r._taskCosts={},r._taskLoad=0,r.postMessage({type:"init",decoderConfig:this.decoderConfig}),r.onmessage=function(a){const t=a.data;switch(t.type){case"decode":r._callbacks[t.id].resolve(t);break;case"error":r._callbacks[t.id].reject(t);break;default:console.error('THREE.DRACOLoader: Unexpected message, "'+t.type+'"')}},this.workerPool.push(r)}else this.workerPool.sort(function(r,a){return r._taskLoad>a._taskLoad?-1:1});const s=this.workerPool[this.workerPool.length-1];return s._taskCosts[e]=o,s._taskLoad+=o,s})}_releaseTask(e,o){e._taskLoad-=e._taskCosts[o],delete e._callbacks[o],delete e._taskCosts[o]}debug(){console.log("Task load: ",this.workerPool.map(e=>e._taskLoad))}dispose(){for(let e=0;e<this.workerPool.length;++e)this.workerPool[e].terminate();return this.workerPool.length=0,this.workerSourceURL!==""&&URL.revokeObjectURL(this.workerSourceURL),this}}function nt(){let n,e;onmessage=function(t){const i=t.data;switch(i.type){case"init":n=i.decoderConfig,e=new Promise(function(d){n.onModuleLoaded=function(p){d({draco:p})},DracoDecoderModule(n)});break;case"decode":const h=i.buffer,l=i.taskConfig;e.then(d=>{const p=d.draco,c=new p.Decoder;try{const u=o(p,c,new Int8Array(h),l),m=u.attributes.map(g=>g.array.buffer);u.index&&m.push(u.index.array.buffer),self.postMessage({type:"decode",id:i.id,geometry:u},m)}catch(u){console.error(u),self.postMessage({type:"error",id:i.id,error:u.message})}finally{p.destroy(c)}});break}};function o(t,i,h,l){const d=l.attributeIDs,p=l.attributeTypes;let c,u;const m=i.GetEncodedGeometryType(h);if(m===t.TRIANGULAR_MESH)c=new t.Mesh,u=i.DecodeArrayToMesh(h,h.byteLength,c);else if(m===t.POINT_CLOUD)c=new t.PointCloud,u=i.DecodeArrayToPointCloud(h,h.byteLength,c);else throw new Error("THREE.DRACOLoader: Unexpected geometry type.");if(!u.ok()||c.ptr===0)throw new Error("THREE.DRACOLoader: Decoding failed: "+u.error_msg());const g={index:null,attributes:[]};for(const b in d){const w=self[p[b]];let y,v;if(l.useUniqueIDs)v=d[b],y=i.GetAttributeByUniqueId(c,v);else{if(v=i.GetAttributeId(c,t[d[b]]),v===-1)continue;y=i.GetAttribute(c,v)}const q=r(t,i,c,b,w,y);b==="color"&&(q.vertexColorSpace=l.vertexColorSpace),g.attributes.push(q)}return m===t.TRIANGULAR_MESH&&(g.index=s(t,i,c)),t.destroy(c),g}function s(t,i,h){const d=h.num_faces()*3,p=d*4,c=t._malloc(p);i.GetTrianglesUInt32Array(h,p,c);const u=new Uint32Array(t.HEAPF32.buffer,c,d).slice();return t._free(c),{array:u,itemSize:1}}function r(t,i,h,l,d,p){const c=p.num_components(),m=h.num_points()*c,g=m*d.BYTES_PER_ELEMENT,b=a(t,d),w=t._malloc(g);i.GetAttributeDataArrayForAllPoints(h,p,b,g,w);const y=new d(t.HEAPF32.buffer,w,m).slice();return t._free(w),{name:l,array:y,itemSize:c}}function a(t,i){switch(i){case Float32Array:return t.DT_FLOAT32;case Int8Array:return t.DT_INT8;case Int16Array:return t.DT_INT16;case Int32Array:return t.DT_INT32;case Uint8Array:return t.DT_UINT8;case Uint16Array:return t.DT_UINT16;case Uint32Array:return t.DT_UINT32}}}var U={url:"kernel/7910a78de525/first-part.json",mesh:"kernel/7910a78de525/first-part.drc",decoder:"kernel/7910a78de525/draco/",part:"twisted-planter",script:`// Twisted planter and its drip saucer, printed as two parts.
//
// The planter is a six-point star lofted through nine sections, each wider
// and turned further than the last, so its walls come out as twisted facets.
// The saucer is a dish of smooth bumps in rings of 1, 6, 12 and 18, which the
...
`};const f=typeof U>"u"?void 0:U;let Z=!1,F;const ee=(n,e)=>!!f&&n===f.part&&e===f.script;async function ot(n,e){if(!f||Z||!ee(n,e))return;Z=!0;const o=await fetch(new URL(f.url,document.baseURI)).catch(()=>{});if(!o?.ok)return;const s=await o.json().catch(()=>{});if(!s)return;const r=await at(f.mesh,f.decoder).catch(()=>{});if(r)return{...s,...r,shipped:!0}}async function at(n,e){const o=await fetch(new URL(n,document.baseURI));if(!o.ok)throw new Error(`the shipped mesh is not there: ${o.status}`);const s=await o.arrayBuffer(),r=new tt().setDecoderPath(new URL(e,document.baseURI).href),a=await new Promise((t,i)=>r.parse(s,t,i));return r.dispose(),{positions:a.getAttribute("position").array,normals:a.getAttribute("normal").array,indices:a.getIndex().array}}function st(n){return F=V(n),F}function rt(n,e){return ee(n,e)?F:void 0}const ct=Object.freeze(Object.defineProperty({__proto__:null,inFlight:rt,rebuild:st,take:ot},Symbol.toStringTag,{value:"Module"}));export{lt as host,ht as kernel,ct as prebuilt};
//# sourceMappingURL=index-Djg7XXf8.js.map
