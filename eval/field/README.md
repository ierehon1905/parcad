...
| [how-thin-is-it](how-thin-is-it.md) | `measure_wall_thickness` | The thinnest wall, which no variable names. `thickness = 19.1` is the wrong answer sitting one line away. The back-face countersinks bring a hole's rim to 4.8 from the OD, which the ray sweep reported and the inscribed ball does not: the wall is the 6.3 mm ligament. PERCEPTION §5. |
| [where-is-the-sliver](where-is-the-sliver.md) | `measure_wall_thickness` | Whether a model reads a `feather` reported at 0 mm as the defect: a floor falling 15° meets a channel's ceiling, the sweep used to quote whichever sample landed nearest the seam, and 5, 6 and 1 are all in the script. The part is in the prompt because every seeded part is free of it. PERCEPTION §5, "What is certain". |
| [how-thick-is-the-shade](how-thick-is-the-shade.md) | `measure_wall_thickness` | Whether a model can tell a wall thickness from a line through the wall: 1.6 is in the script and on a horizontal probe, the ball across the sloped wall is 1.552, and the open rims are listed as thin `edge` places a model has to read past. PERCEPTION §5. |
...
| [show-it-in-my-browser](show-it-in-my-browser.md) | `save_project`, `open_project` | Asked to see a part it built, does a model put it on the screen the user is watching, as its own project, rather than drawing it some other way or writing it over the part that was open? Written from a claude.ai session on ParCAD web that built its own viewer instead. Writes a project; run against a tab whose folder is reset between trials, one at a time. |
...
| [change-one-dimension](change-one-dimension.md) | `edit_part` | Editing in place: one dimension of a saved 6.5 KB part changed and saved, with `input` refusing any `script` argument of 2 KB or more — a trial that resends the whole part is LUCKY at best, and the scorer's `sent` column is the bytes it cost. The part is `examples/fusion360/retainer-v1.js` seeded as `disc-retainer`; it is edited in place, so one trial at a time with the copy put back between trials. docs/PERCEPTION.md §19. |
| [what-if-it-were-thicker](what-if-it-were-thicker.md) | `evaluate_part` `project` + `edits` | The what-if: the volume at another thickness from one `{ project, edits }` call, with `list_snapshots` proving nothing was written. Same seeded part under `slot-retainer`, one trial at a time on a fresh copy. |
...
| [is-this-ready-to-print](is-this-ready-to-print.md) | `evaluate_part` `print_check` | The check that runs without being asked (docs/NEXT.md, item 1): a part that is watertight, flat and fits the bed, hiding a 0 mm feather between two tagged features and a grille cut into a screw boss. SOUND only when the reply tells the user both. The part is `eval/scripts/nicked-stand.js` seeded as `cable-stand`. |
| [where-does-it-need-support](where-does-it-need-support.md) | `evaluate_part` `print_check` overhang | Overhang per body in its print orientation, written before the feature existed (docs/COIN_HOLDER_REVIEW.md, workflow §11): eight lips at 0° inside two cups, invisible to every printability number before it. The part is `eval/scripts/coin-cups.js` seeded as `coin-cups`. |
