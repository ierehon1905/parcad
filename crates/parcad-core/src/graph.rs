...
pub fn surface_loft_extent(sections: &[CurveSection], resolved: &[Section], smooth: bool) -> (V3, V3) {
    ...
    surface_loft_extent_of(sections, &boxes, smooth)
}

/// [`surface_loft_extent`] from each curve's box as the backend built it: a
/// fitted curve can reach past its points between two of them.
pub fn surface_loft_extent_of(sections: &[CurveSection], boxes: &[([f64; 2], [f64; 2])], smooth: bool) -> (V3, V3) {
    let (mut lo, mut hi) = ([f64::MAX; 2], [f64::MIN; 2]);
    for (a, b) in boxes {
        lo = [lo[0].min(a[0]), lo[1].min(a[1])];
        hi = [hi[0].max(b[0]), hi[1].max(b[1])];
