...
fn measure(
    ...
    // The integral itself misreads walls of many poles (docs/GOTCHAS.md, "The
    // volume integral misreads a wavy B-spline wall"): a thickened pleated
    // shade read 3 % low. Before refusing, ask the exact question the volume
    // stands in for — does every face's mesh cover the face — in each face's
    // own parameter plane, where no surface integral is needed.
