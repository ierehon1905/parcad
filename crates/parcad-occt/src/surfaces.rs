...
use parcad_core::skin::{height_parameters, uniform_cubic_knots, Surface, CORRECTION_ROUNDS};

/// Relative error a stitched solid's volume is integrated to: it is only
/// asked whether the shell encloses anything.
const STITCH_VOLUME_EPS: f64 = 1e-3;
...
/// one, and is refused before that: past this the offset skin moves at under
/// an eighth of the surface's speed, and the mesher leaves holes in it
/// (docs/GOTCHAS.md, "A thickened fold meshed open").
const FOLD_LIMIT: f64 = 0.88;

/// Points per continuous stretch of a surface, each way, that the search for
/// its tightest bend starts from.
const BEND_SAMPLES: usize = 4;
...
/// Every open curve fitted on one shared knot vector at one parameter per
/// point, the span count doubling from four until each holds its tolerance
/// without looping, and the surface interpolated across them, as
/// [`surface_bands`]. Returns the shell, its exact box, the worst deviation
/// and each curve's box. Closed curves are fitted as a skinned loft fits its sections,
/// by [`crate::skinned::outside`].
fn open_skinned_surface(
    sections: &[(&[P2], f64, f64)],
    smooth: bool,
    params: &[f64],
    label: &str,
) -> Result<(Shape, (DVec3, DVec3), f64, Vec<(P2, P2)>)> {
    let points: Vec<&[P2]> = sections.iter().map(|s| s.0).collect();
    let mut spans = 4;
    let mut short: Option<String> = None;
    loop {
        let mut fit = match OpenFit::new(params, spans) {
            Ok(fit) => fit,
            Err(e) => match short {
                Some(why) => bail!(
        ...
        let mut best: Option<(f64, OpenFit)> = None;
        for round in 0..=CORRECTION_ROUNDS {
            ...
            if round == 0 && off > HOPELESS * tolerance && OpenFit::new(params, spans * 2).is_ok() {
                best = Some((off, fit));
                break;
            }
            ...
            let current = fit;
            match next.map(|p| OpenFit::new(&p, spans)) {
                Some(Ok(f)) => {
                    if better {
        ...
        let (shell, bounds) = surface_bands(&surface, &vparams, fit.spans(), label)?;
        breadcrumb(&format!(
            "{label}: {} curves fitted on one knot vector of {} spans, {worst:.4} mm off at worst",
            sections.len(),
            fit.spans()
        ));
        return Ok((shell, bounds, worst, extents));
    }
}

/// `surface` as a shell of one face per stretch between curves, each cut
/// along u into pieces of at most [`SPANS_PER_FACE`] of its `spans`, and the
/// shell's exact box.
fn surface_bands(surface: &Surface, vparams: &[f64], spans: usize, label: &str) -> Result<(Shape, (DVec3, DVec3))> {
...
fn fitted_curves<'a>(sections: &[CurveSection], resolved: &'a [Section]) -> Option<Vec<(&'a [P2], f64, f64)>> {
    let out: Vec<(&[P2], f64, f64)> = sections
        ...
        .collect::<Option<_>>()?;
    let n = out[0].0.len();
    (n >= crate::skinned::MIN_POINTS && out.iter().all(|s| s.0.len() == n)).then_some(out)
}
...
pub(super) fn surface_loft(node: &Node, id: NodeId, offset: DVec3, sections: &[CurveSection], closed: bool, smooth: bool) -> Result<BuiltShape> {
    ...
    // Each curve's box as built: a fitted curve can reach past its points
    // between two of them, where a smooth curve has its extreme.
    let mut curve_boxes: Vec<(P2, P2)> = resolved.iter().map(Section::bounds).collect();
    let shape = match fitted_curves(sections, &resolved) {
        Some(fits) => {
            ...
            let (shape, bounds, deviation) = if closed {
                ...
                let sections: Vec<crate::skinned::FitSection> =
                    fits.iter().map(|&(points, tolerance, z)| crate::skinned::FitSection { points, tolerance, z }).collect();
                let outside = crate::skinned::outside(&sections, smooth, None, &who)?;
                let (shell, bounds) = surface_bands(&outside.surface, &outside.vparams, outside.spans(), &who)?;
                curve_boxes = outside.section_extents().to_vec();
                (shell, bounds, outside.deviation_mm)
            } else {
                let (shell, bounds, deviation, extents) = open_skinned_surface(&fits, smooth, &open_shared_parameters(&points), &who)?;
                curve_boxes = extents;
                (shell, bounds, deviation)
            };
        ...
        None => {
            ...
            for ((curve, wire), entry) in resolved.iter().zip(&wires).zip(curve_boxes.iter_mut()) {
                if let Some((lo, hi)) = wire.to_shape().bounds_optimal().filter(|_| curve.has_fit()) {
                    *entry = ([lo.x, lo.y], [hi.x, hi.y]);
                }
            }
    ...
    let (lo, hi) = parcad_core::graph::surface_loft_extent_of(sections, &curve_boxes, smooth);
    let (lo, hi) = (v(lo), v(hi));
...
pub(super) fn stitch(doc: &Doc, node: &Node, id: NodeId, offset: DVec3, children: &[NodeId], tolerance: f64, solid: bool) -> Result<BuiltShape> {
    ...
    if census.free_edges == 0 && census.shells == 1 {
        let Some(closed) = sewn.closed_solid() else {
            bail!("node {id} ({label}): the stitched surfaces close, but not into one shell the kernel can make a solid of. Check that no surface passes through another");
        };
        if let Err(report) = closed.check_validity(false) {
            bail!(
                "node {id} ({label}): the stitched shell closes, and its solid does not pass the kernel's own check ({}). Check that no surface passes through another",
                report.lines().take(3).collect::<Vec<_>>().join("; ")
            );
        }
        let closed = facing_outward(closed, &|| format!("node {id} ({label})'s stitched solid"))?;
        // Span by span: the whole-face integral misreads B-spline walls
        // (docs/GOTCHAS.md, "The volume integral misreads a thickened pleat").
        let (volume, _) = closed.volume_by_spans(STITCH_VOLUME_EPS).map_err(kernel(id, label))?;
        if !(volume > 0.0) {
...
fn refuse_fold(id: NodeId, label: &str, doing: &str, amount: f64, side: &str, found: (FaceSample, f64)) -> anyhow::Error {
    ...
    let most = FOLD_LIMIT * radius;
    let least_radius = amount / FOLD_LIMIT;
    anyhow::anyhow!(
        "node {id} ({label}) {doing} by {amount} mm on {side}, but the surface bends too tightly for that near ({:.2}, {:.2}, {:.2}): its radius of curvature there is {radius:.3} mm on that side. An offset past its own radius folds through itself, and one within an eighth of it leaves a skin turning so tightly that the mesh cannot close over it. Use at most {most:.2} mm there, thicken toward the other side, or round the curve that bends there to at least {least_radius:.2} mm — a fitted curve can turn tighter between its points than the points do",
...
pub(super) fn thicken(doc: &Doc, node: &Node, id: NodeId, offset: DVec3, child: NodeId, thickness: f64, side: ThickenSide) -> Result<BuiltShape> {
    ...
    let bends = base.shape.bend_extremes(BEND_SAMPLES).map_err(kernel(id, label))?;
    breadcrumb(&format!("thicken node {id}: {} samples taken", samples.len()));
    if let Some(found) = (along > 0.0).then(|| fold(&bends, along)).flatten() {
        return Err(refuse_fold(id, label, "thickens", along, "its normal side", found));
    }
    if let Some(found) = (against > 0.0).then(|| fold(&bends, -against)).flatten() {
    ...
    let solid = solid.single_solid().unwrap_or(solid);
    let solid = facing_outward(solid, &|| format!("node {id} ({label})'s thickened solid"))?;
...
pub(super) fn offset_surface(doc: &Doc, node: &Node, id: NodeId, offset: DVec3, child: NodeId, distance: f64) -> Result<BuiltShape> {
    ...
    let samples = base.shape.face_samples(SAMPLES_PER_FACE).map_err(kernel(id, label))?;
    let bends = base.shape.bend_extremes(BEND_SAMPLES).map_err(kernel(id, label))?;
    if let Some(found) = fold(&bends, distance) {
...
mod tests {
    ...
    /// A surface of `pleats` pleats `depth` deep round a 60 mm circle, the
    /// same fitted curve at three heights: a cylinder over a wavy B-spline.
    fn pleated_cylinder(pleats: f64, depth: f64, height: f64) -> serde_json::Value {
        let n = 480;
        let curve: Vec<[f64; 2]> = (0..n)
            .map(|i| {
                let a = std::f64::consts::TAU * i as f64 / n as f64;
                let r = 60.0 + depth * (pleats * a).cos();
                [r * a.cos(), r * a.sin()]
            })
            .collect();
        let sections: Vec<serde_json::Value> = [0.0, height / 2.0, height]
            .iter()
            .map(|z| serde_json::json!({ "z": z, "curve": [{ "fit": curve, "tolerance": 0.05 }] }))
            .collect();
        serde_json::json!({ "op": "surface_loft", "sections": sections, "closed": true, "smooth": true })
    }

    #[test]
    fn a_wall_that_leaves_its_inside_skin_all_but_folded_is_refused_before_it_meshes_open() {
        // The fitted curve of 24 pleats 6 mm deep turns at 0.740 mm between its
        // points (0.857 mm on the curve it was sampled from), so half of a
        // 1.4 mm wall leaves a skin turning at 0.04 mm. Built, the mesher left
        // holes in five of its faces; 5.5 mm pleats, 0.82 mm, mesh closed.
        let wall = |depth: f64| {
            build_part(&doc(serde_json::json!({ "root": 1, "nodes": [
                pleated_cylinder(24.0, depth, 200.0),
                { "op": "thicken", "child": 0, "thickness": 1.4 }
            ] })))
        };
        let refused = wall(6.0).err().expect("a wall 0.04 mm from folding is refused").to_string();
        assert!(refused.contains("radius of curvature there is 0.740 mm"), "{refused}");
        let built = wall(5.5).unwrap();
        let _ = built.shape.mesh();
        assert!(built.shape.uncovered_faces(1e-6).unwrap().is_empty());
    }

    #[test]
    fn a_mesh_covers_faces_with_seams_poles_and_a_tiny_parameter_range() {
        let parts = serde_json::json!({ "root": 3, "nodes": [
            { "op": "cylinder", "r": 5.0, "h": 10.0 },
            { "op": "sphere", "r": 4.0 },
            { "op": "torus", "major": 20.0, "minor": 3.0 },
            { "op": "union", "children": [0, 1, 2], "blend": 0.0 }
        ] });
        // A sliver of a cylinder's side, its parameters spanning 2e-5 rad and
        // 2e-3 mm about (π/2, 1).
        let sliver = serde_json::json!({ "root": 3, "nodes": [
            { "op": "cylinder", "r": 100.0, "h": 10.0 },
            { "op": "cuboid", "size": { "x": 0.002, "y": 0.2, "z": 0.002 } },
            { "op": "translate", "child": 1, "by": { "x": 0.0, "y": 100.0, "z": 1.0 } },
            { "op": "intersection", "children": [0, 2], "blend": 0.0 }
        ] });
        for json in [parts, sliver] {
            let part = build_part(&doc(json)).unwrap();
            let _ = part.shape.mesh();
            let uncovered = part.shape.uncovered_faces(1e-6).unwrap();
            assert!(uncovered.is_empty(), "{uncovered:?}");
        }
    }

    #[test]
    fn a_thickened_pleat_reports_the_volume_its_closed_form_gives() {
        // Every section is the same curve, so the surface is a cylinder over
        // it: no Gaussian curvature, and a wall centred on it, closed along its
        // normals, holds t × L × h exactly, L being the curve's length — half
        // the surface's free edge length.
        let (t, h) = (1.4, 200.0);
        let surface = build_part(&doc(serde_json::json!({ "root": 0, "nodes": [pleated_cylinder(24.0, 4.0, h)] }))).unwrap();
        let length = surface.shape.census().unwrap().free_edge_length / 2.0;
        let closed_form = t * length * h;
        let thick = build_part(&doc(serde_json::json!({ "root": 1, "nodes": [
            pleated_cylinder(24.0, 4.0, h),
            { "op": "thicken", "child": 0, "thickness": t }
        ] })))
        .unwrap();
        let integral = thick.shape.signed_volume();
        let (by_spans, _) = thick.shape.volume_by_spans(1e-4).unwrap();
        let mesh = thick.shape.mesh();
        ...
        let uncovered = thick.shape.uncovered_faces(1e-6).unwrap();
        eprintln!(
            "closed form {closed_form:.1} mm³ (L = {length:.4} mm); mesh {volume:.1}; span by span {by_spans:.1}; whole-face integral {integral:.1} ({:+.1} %)",
            100.0 * (integral / closed_form - 1.0),
        );
        // The integral the mesh backstop compares with is far off on this
        // wall, so the backstop must ask the mesh whether it covers each face
        // — it does — and the part reports the mesh's volume.
        assert!((integral - closed_form).abs() > 0.1 * closed_form, "the whole-face integral now reads {integral}: the backstop's relaxation may be unneeded");
        assert!((by_spans - closed_form).abs() < 1e-4 * closed_form, "span by span {by_spans} against {closed_form}");
        assert!(uncovered.is_empty(), "{uncovered:?}");
        // A chord of the 0.01 mm mesh lies at most that far off its surface.
        assert!((volume - closed_form).abs() <= area * 0.01, "mesh {volume} against {closed_form}, allowed {}", area * 0.01);
        assert!((volume - closed_form).abs() < 1e-3 * closed_form, "mesh {volume} against {closed_form}");
    }
