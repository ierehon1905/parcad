...
EMSDK=/path/to/emsdk web/build-kernel.sh   # ParCAD web's two wasm modules; web/README.md
(cd relay && bun test test)            # the relay under wrangler dev
...
the two together is how a suite comes to measure the wrong thing — the most
...
| where a section's arcs and curves cross | `crates/parcad-core/src/section_crossing.rs` — fitted curves too, once the kernel has built them (`fitted_crossing`) |
