...
6. **What the coin-holder session cost, in order** — reviewed 2026-09-18 from
   ...
   - a one-day correctness batch: an edge count on every treatment, the
     shadowed builtin named, `rotate` arguments validated, no script echo
     from `open_project` (`deny_unknown_fields` on every request struct,
     `--set` coerced by schema and dead viewers expired landed 2026-09-18);
