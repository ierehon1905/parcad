...
6. **What the coin-holder session cost, in order** — reviewed 2026-09-18 from
   ...
   - a one-day correctness batch: an edge count on every treatment, the
     shadowed builtin named, `rotate` arguments validated
     (`deny_unknown_fields` on every request struct, `--set` coerced by
     schema, dead viewers expired and the script echo dropped from
     `open_project` and `set_script` landed 2026-09-18);
