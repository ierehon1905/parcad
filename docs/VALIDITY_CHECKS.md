...
| loft (bounding-box bulge; volume sign) | walls passing through each other | a half-turn loft, a star turned three corners on (built with volume −3382 mm³), a curved loft through its axis — all accepted; an L turned past its notch caught only by the mesh backstop | ruled walls between polygons: exact, in the graph (`loft_walls`); a loft through fitted sections (skinned, walled or not): exact, on the skins' poles (`skin_crossing`, below), the kernel's check only where that leaves something unsettled; anything else curved or smooth: the kernel's self-intersection check on the loft | exact checks ≈ 0 (the pleated shade: 0.12 s against 16.4 s for the kernel's); other smooth/curved lofts ≈ one check each (loft-smooth 17 ms) |
...
| orientation: loft, sweep, curved extrude, thread, offset (volume sign); nothing after the last node | an inside-out solid | a walled smooth loft (integration, 2026-09-16) passed every gate; a point outside it probed as material | a skinned loft states its outside and checks the face it presents there (`Skinner::set_outward`, docs/ARCHITECTURE.md), and is not classified again; `facing_outward` at every other operation that builds a solid from surfaces — `thicken` and a `stitchSurfaces` that closes included; after the last node, each closed mesh shell's winding checked against its nesting (`Tessellation::inward_shells`), which also covers a whole mesh of negative volume; probes and fit checks, which make no mesh, ask the B-rep (`check_finished`) | per-operation classification replaces the volume integral it used; the mesh check is free |
| mesh backstops in `serve::measure` | a surface that does not close; a closed mesh of the wrong solid | unchanged | unchanged, plus the orientation backstop above; where the mesh and the volume integral disagree, each face's mesh is asked whether it covers the face (`Shape::uncovered_faces`), because the integral misreads B-spline walls (docs/GOTCHAS.md) | — |
...
## A skinned loft's crossings, on its poles

A loft through fitted sections has its height linear in `v` on every skin
(docs/ARCHITECTURE.md, "A walled loft"), so two of a skin's points at one
height are at one `v`. A skin then crosses itself only where one of its
horizontal cuts, a closed plane curve, does, and the outside meets the inside
only where their cuts at one height do. `skin_crossing::skins_apart` settles
both on Bézier patches of the skins: a patch whose `u`-derivative control
vectors keep to an open half-plane cuts every height in a curve running one
way, so it cannot cross itself, nor a patch beside it in `u` when the two
together still keep to one; two patches whose control boxes are more than
OpenCASCADE's confusion distance apart cannot meet. Anything else is halved.
A pair that stays unsettled when small is solved by Newton for a point the two
cuts share, which, found, is the refusal; one that neither settles nor solves
(a cusp, a touch within tolerance) is handed to the kernel's check on the
built solid. Every skin's poles are checked to have one height per row, rising
with `v`, or the kernel decides.

The kernel's check spent 16.4 s of `walled-twisted-pleats` — every second of
it `IntTools_FaceFace` intersecting each band with itself — on a part that
builds in 6–12 s; the poles take 0.12 s. On five walled corpus lofts they
take 0.7–12 ms. Checked against the kernel on the same solids: a star lofted
to itself 0, 3, 7, 8 and 11 points round, ruled and smooth (the unit test
`the_skins_crossing_check_agrees_with_the_kernels`), and the same star through
the DSL at eight shifts, where both said clear at 7, 10, 11 and 13 and both
found the crossing at 8, 9 and 15 (`refuse-fitted-loft-crossing` keeps 8).
...
- A fillet or chamfer node does not run `BRepCheck`, as a blend does, nor
  ...
  whole (1).
- The walled smooth loft that prompted the orientation gate
  (`walled-twisted-pleats`) now builds facing outward under its own stated
  side, and no longer reproduces the classifier's error; the gate was tested
  on solids reversed on purpose and on the offset of a filleted body, and the
  mesh backstop is what would catch a recurrence.
