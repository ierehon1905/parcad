...
| orientation: loft, sweep, curved extrude, thread, offset (volume sign); nothing after the last node | an inside-out solid | a walled smooth loft (integration, 2026-09-16) passed every gate; a point outside it probed as material | a skinned loft states its outside and checks the face it presents there (`Skinner::set_outward`, docs/ARCHITECTURE.md), and is not classified again; `facing_outward` at every other operation that builds a solid from surfaces; after the last node, each closed mesh shell's winding checked against its nesting (`Tessellation::inward_shells`), which also covers a whole mesh of negative volume; probes and fit checks, which make no mesh, ask the B-rep (`check_finished`) | per-operation classification replaces the volume integral it used; the mesh check is free |
